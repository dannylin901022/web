...
//                顯示收藏
        ...
        for(var i = 0;i<data.length;i++){
            collect_list += ("<a href='#sublist" + i +"' data-bs-toggle='collapse' id='dropdown' aria-expanded='false' style='font-size:30px;margin-left:20px;'>" + data[i].favoriteFolderName + "</a><ul id='sublist" + i +"'  class='list-unstyled collapse'>");
            
            for(var j = 0;j<data[i].favoriteFolderItems.length;j++){
                collect_list += ("<li><a href='#' id='dropdown_folder' style='text-decoration: none;font-size: 20px; margin-left:50px; line-height: 2; color: #555555;  font-family: 'Noto Sans TC', sans-serif;'>" + data[i].favoriteFolderItems[j].favoriteItem.favoriteItemName + "</a></li>");
            }
...
//                更新資料夾名稱
    async collect_patch_data(api_url,token,string,id) {
                    ...
                    await fetch(api_url + "FavoriteFolders/"+id+"?favoriteFolderName="+string,{
                        method: "PATCH",
                        headers: {
                            "Authorization": 'Bearer ' +  token,
                            "Content-type": 'application/json',
